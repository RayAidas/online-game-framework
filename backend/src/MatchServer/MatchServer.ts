...
`);
	...
	async joinRoomServer(serverUrl: string): Promise<void> {
		...
		client.listenMsg("clientMsg/UpdateRoomState", (msg) => {
		...
		try {
			...
			// Auth as MatchServer
			let op2 = await client.callApi("Auth", {
	...
	async startMatch() {
		...
		setTimeout(() => {
			this.startMatch();
		}, BackConfig.matchServer.intervalMatch);
	}
	...
	private async _doMatch() {
		...
		// 优先匹配更早开始匹配的房间
		let matchingRooms = this.roomServers
			.map((v) => {
