const app = new Vue({
    ...
    methods: {
        ...
        updateIncreaseData() {
            let ids = [];
            ids= this.$data.initData.map(e => e.id);
            this.$data.increaseData = this.$data.cacheRawData.filter(item => !ids.includes(item.id))
        },
        openCacheDialog() {
            [this.$data.cacheRawData, this.$data.cacheTitle] = cc.cache();
            this.updateCacheData();
            if (!this.$data.initData.length) this.$data.initData = this.$data.cacheRawData;
            this.updateIncreaseData()
